...
export function generateDeterministicInvestigationReport(context:InvestigationContext):InvestigationReport{
  ...
  if(isResolved){
    ...
    recommendedAction="No manual operational intervention required. Reconciled flow is ready for downstream ledger posting and settlement closure.";
    ...
    recommendedAction="Hold automated batch payout. An operations analyst must inspect external gateway metadata and batch settlement IDs to break the tie.";
    questionsToInvestigate.push(
    ...
    recommendedAction="Flag for exception review. Investigate whether the downstream settlement or bank entry was delayed beyond 7 days or missing from provider clearing files.";
    questionsToInvestigate.push(
...
export function generateDeterministicAskAnswer(question:string,context:InvestigationContext):AskInvestigationResponse{
  ...
  if(q.includes("risk")||q.includes("level")||q.includes("severity")){
    if(isResolved){
      return{
        answer:`Risk is LOW. The match is unique and meets all deterministic criteria (+${result.evidenceScore} evidence score). Automated posting is safe.`,
        provider:"deterministic-fallback"
      };
    }
    if(isAmbiguous){
      return{
        answer:`Risk is HIGH. Automated posting is blocked because assigning ${record.id} to the wrong candidate among ${result.candidateRecordIds.join(", ")} would corrupt ledger reconciliation.`,
        provider:"deterministic-fallback"
      };
    }
    return{
      answer:`Risk is MEDIUM/HIGH. Amount ${formattedAmount} is unreconciled. Operational verification is needed to confirm if the counterpart transaction is missing or delayed.`,
      provider:"deterministic-fallback"
    };
  }
  if(q.includes("action")||q.includes("next")||q.includes("do")||q.includes("check")||q.includes("analyst")){
    ...
    if(isAmbiguous){
      return{
        answer:`Recommended action: Hold payout. Cross-check merchant batch IDs and authorization codes across candidates ${result.candidateRecordIds.join(" and ")} in the provider gateway dashboard.`,
        provider:"deterministic-fallback"
      };
    }
    return{
      answer:`Recommended action: Inspect clearing files for the next 7 days or query the payment gateway for refund/cancellation status on record ${record.id}.`,
      provider:"deterministic-fallback"
    };
  }
...
async function callGeminiApi(prompt:string,apiKey:string,timeoutMs:number=10000):Promise<string>{
  const models=["gemini-3.1-flash-lite","gemini-3.5-flash-lite","gemini-3.1-flash-lite"];
...
Your job is to provide an explanatory audit report for a deterministic reconciliation result.
CRITICAL RULES:
...
3. Return STRICT JSON conforming to this schema:
{
  ...
  "recommendedAction": "Concrete next step for operations team",
  ...
  try{
    ...
    if(
      ...
      typeof parsed.whyThisStatus==="string"&&
      Array.isArray(parsed.keyEvidence)&&
      (parsed.riskLevel==="LOW"||parsed.riskLevel==="MEDIUM"||parsed.riskLevel==="HIGH")&&
      typeof parsed.recommendedAction==="string"&&
      Array.isArray(parsed.questionsToInvestigate)
    ){
      return{
        summary:parsed.summary,
        whyThisStatus:parsed.whyThisStatus,
        keyEvidence:parsed.keyEvidence.map(String),
        riskLevel:parsed.riskLevel,
        recommendedAction:parsed.recommendedAction,
        questionsToInvestigate:parsed.questionsToInvestigate.map(String),
        provider
      };
    }
